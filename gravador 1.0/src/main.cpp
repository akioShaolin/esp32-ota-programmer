...
// ============================================================
// GravadorESP 1.1
// Rede + AP de manutenção + mDNS + página web + upload do .bin
//
// IMPORTANTE:
// O LED verde de RS-485 (GPIO13) NÃO é utilizado.
// O GPIO13 permanece como INPUT_PULLDOWN.
//
// Esta versão ainda não transmite o firmware por ArduinoOTA.
// Ela valida toda a infraestrutura anterior ao protocolo OTA.
// ============================================================
...
<div class="grid">
  ...
  <section class="card full">
    ...
    <button onclick="uploadFirmware()">Enviar firmware</button>
    ...
    <div class="stats" style="margin-top:12px">
      ...
      <div class="stat"><span>Transmissão OTA</span><strong>Próxima etapa</strong></div>
    </div>
    <button class="danger" onclick="deleteFirmware()">Excluir firmware armazenado</button>
  </section>
</div>

<footer>GravadorESP 1.1 · GPIO13/LED verde RS-485 desativado</footer>
</main>
...
void loadPreferences() {
    ...
    targetHost = prefs.getString("ota_host", "");
    targetOtaPassword = prefs.getString("ota_pass", "");
    targetPort = prefs.getUShort("ota_port", 8266);
    targetTimeoutSeconds = prefs.getUInt("ota_timeout", 10);

    firmwareMd5 = prefs.getString("fw_md5", "");
    uploadOriginalName = prefs.getString("fw_name", "");
}
...
void handleStatus() {
    const bool firmwareExists = littleFsReady && LittleFS.exists(FIRMWARE_PATH);
    size_t firmwareSize = 0;

    if (firmwareExists) {
        File file = LittleFS.open(FIRMWARE_PATH, FILE_READ);
        if (file) {
...
void updateLeds() {
    ...
    digitalWrite(Pins::LED_RS485_RED,
                 static_cast<int32_t>(rsRedPulseUntilMs - now) > 0 ? HIGH : LOW);
}
...
void handleButton() {
    ...
    if (stableButton == LOW) {
        ...
        if (littleFsReady && LittleFS.exists(FIRMWARE_PATH)) {
            Serial.println("[BOTAO] Firmware armazenado. A transmissão OTA será a próxima etapa.");
        } else {
            Serial.println("[BOTAO] Nenhum firmware armazenado.");
        }
    }
}
...
void printStartupSummary() {
    Serial.println();
    Serial.println("============================================================");
    Serial.println(" GravadorESP 1.1 - Rede, pagina e armazenamento");
    Serial.println("============================================================");
...
void onWiFiEvent(WiFiEvent_t event, WiFiEventInfo_t info) {
    switch (event) {
        case ARDUINO_EVENT_WIFI_STA_CONNECTED:
            Serial.println("[WiFi] Associado ao access point.");
            break;
        ...
        case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:
            Serial.printf(
                "[WiFi] Desconectado. Motivo numerico: %u\n",
